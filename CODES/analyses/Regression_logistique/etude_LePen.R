...
dataset$LEP <- FALSE
dataset$LEP[dataset$`Marine LE PEN` >= 0.3] <- TRUE
...
reg <- glm(LEP ~ p_chom_15_64 + p_CS3_15_64 + p_CS1_15_64 +
             p_CS2_15_64 + p_CS4_15_64 +
             prix_moyen_m2 + RFR_2019 +
             p_proprios + p_res_secondaires +
             p_appartements + popu + p_1919_1945 +
             p_log_120_supm2 +
...
for (k in c(1:10)){
  ...
  B$p_A = NULL
  A$p_A = NULL
  reg <- glm(LEP ~ p_chom_15_64 + p_CS3_15_64 + p_CS6_15_64 +
               p_CS4_15_64 +
               prix_moyen_m2 + RFR_2019  +
  ...
  X = table(LEP.pred > 0.5 , A$`Marine LE PEN` >= 0.3)
