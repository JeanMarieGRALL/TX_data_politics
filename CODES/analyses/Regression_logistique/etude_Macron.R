...
dataset$MAC <- FALSE
dataset$MAC[dataset$`Emmanuel MACRON` >= 0.3] <- TRUE
...
reg <- glm(MAC ~ p_chom_15_64 + p_CS3_15_64 + p_CS6_15_64 + p_CS1_15_64 +
             prix_moyen_m2 + RFR_2019 + nb_delits_p_1000 +
             p_locataires_HLM + p_proprios + p_res_secondaires +
             p_appartements + p_av_1919 +
             p_blancs + p_abstentions,
             data = dataset,
...
# 
# table(MEL.pred > 0.5, dataset$ZEM)
...
plot(ggeffect(reg, "p_chom_15_64"))
...
for (k in c(1:10)){
  ...
  B$p_A = NULL
  A$p_A = NULL
  reg <- glm(MAC ~ p_chom_15_64 + p_CS3_15_64 + p_CS6_15_64 +
               p_CS4_15_64 +
  ...
  X = table(LEP.pred > 0.5 , A$`Emmanuel MACRON` >= 0.3)
...
cor.test(dataset$p_locataires_HLM, dataset$nb_delits_p_1000, method=c("pearson", "kendall", "spearman"))
